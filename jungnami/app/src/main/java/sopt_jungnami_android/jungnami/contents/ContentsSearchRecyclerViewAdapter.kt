...
class ContentsSearchRecyclerViewAdapter(val ctx : Context, val dataList : ArrayList<ContentsSearchData>) : RecyclerView.Adapter<ContentsSearchRecyclerViewAdapter.ContentsSearchResultHolder>() {



    override fun onBindViewHolder(holder: ContentsSearchRecyclerViewAdapter.ContentsSearchResultHolder, position: Int) {
        //마진 동적 설정
        ...
        if ((position%2) == 0){
            ...
            rootLayoutParams.topMargin = (6*dp).toInt()
        } else {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ContentsSearchRecyclerViewAdapter.ContentsSearchResultHolder{
        val view =  LayoutInflater.from(ctx).inflate(R.layout.rv_item_contents, parent, false)
        view.setOnClickListener(onItemClick)
        return ContentsSearchResultHolder(view)
    ...
    inner class ContentsSearchResultHolder(itemView : View) : RecyclerView.ViewHolder(itemView){
